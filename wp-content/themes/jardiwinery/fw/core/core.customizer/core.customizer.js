...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.jardiwinery_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.jardiwinery_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && jardiwinery_get_cookie('to_demo_fixed')==1) jardiwinery_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.jardiwinery_options_field_info .to_demo_pin', function(e) {
		jardiwinery_options_fix_demo(jQuery(this).parents('.jardiwinery_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	jardiwinery_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.jardiwinery_options_input,.jardiwinery_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.jardiwinery_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		jardiwinery_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.jardiwinery_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.jardiwinery_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .jardiwinery_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.jardiwinery_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? JARDIWINERY_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.jardiwinery_options_tab').find('> .jardiwinery_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="jardiwinery_options_schemes_order" class="jardiwinery_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function jardiwinery_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.jardiwinery_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.jardiwinery_options').find('>.jardiwinery_options_field_info');
...
function jardiwinery_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.jardiwinery_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	jardiwinery_message_confirm(JARDIWINERY_STORAGE['to_strings']['scheme_delete_confirm'], JARDIWINERY_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'jardiwinery_options_scheme_delete',
			nonce: JARDIWINERY_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.jardiwinery_options_tab').find('> .jardiwinery_options_schemes_order').val()
		};
		setTimeout(function(){
			jardiwinery_message_info(JARDIWINERY_STORAGE['to_strings']['recompile_styles'], JARDIWINERY_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(JARDIWINERY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				jardiwinery_message_success(JARDIWINERY_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+JARDIWINERY_STORAGE['to_strings']['reload_page'], JARDIWINERY_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				jardiwinery_message_warning(JARDIWINERY_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, JARDIWINERY_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function jardiwinery_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.jardiwinery_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	jardiwinery_message_confirm(JARDIWINERY_STORAGE['to_strings']['scheme_copy_confirm'], JARDIWINERY_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'jardiwinery_options_scheme_copy',
			nonce: JARDIWINERY_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.jardiwinery_options_tab').find('> .jardiwinery_options_schemes_order').val()
		};
		...
		jQuery.post(JARDIWINERY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				jardiwinery_message_success(JARDIWINERY_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+JARDIWINERY_STORAGE['to_strings']['reload_page'], JARDIWINERY_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				jardiwinery_message_warning(JARDIWINERY_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, JARDIWINERY_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function jardiwinery_options_fix_demo(info) {
	var info2 = info.parents('.jardiwinery_options').toggleClass('to_demo_fixed').find('>.jardiwinery_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.jardiwinery_options').append(clonedObj);
		info2 = info.parents('.jardiwinery_options').find('>.jardiwinery_options_field_info');
	}
	jardiwinery_set_cookie('to_demo_fixed', jQuery('.jardiwinery_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function jardiwinery_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.jardiwinery_options_field_info').data('scheme');
