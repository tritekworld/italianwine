...
if ( !function_exists( 'jardiwinery_core_customizer_load_options' ) ) {
    ...
    function jardiwinery_core_customizer_load_options() {
        ...
        if ($mode!='reset' || $override!='customizer') {
            ...
            if (!empty($storage)) jardiwinery_storage_set('custom_colors', $storage);
            $storage = get_option( jardiwinery_storage_get('options_prefix') . '_options_custom_fonts' );
            if (!empty($storage)) jardiwinery_storage_set('custom_fonts', $storage);
        }
    }
}
...
if ( !function_exists( 'jardiwinery_core_customizer_page' ) ) {
	function jardiwinery_core_customizer_page() {
		...
		if (is_array($colors) && count($colors) > 0) {
			...
			$demo_block = '';
            if (jardiwinery_get_theme_setting('customizer_demo') && file_exists(JARDIWINERY_FW_PATH . 'core/core.customizer/core.customizer.demo.php')) {
				ob_start();
                require_once JARDIWINERY_FW_PATH . 'core/core.customizer/core.customizer.demo.php';
...
if (!function_exists('jardiwinery_core_customizer_load_scripts')) {
	function jardiwinery_core_customizer_load_scripts() {
		if (file_exists(jardiwinery_get_file_dir('core/core.customizer/core.customizer.css')))
			wp_enqueue_style( 'jardiwinery-core-customizer-style',	jardiwinery_get_file_url('core/core.customizer/core.customizer.css'), array(), null);
		if (file_exists(jardiwinery_get_file_dir('core/core.customizer/core.customizer.js')))
			wp_enqueue_script( 'jardiwinery-core-customizer-script', jardiwinery_get_file_url('core/core.customizer/core.customizer.js'), array(), null, true );
	}
}
