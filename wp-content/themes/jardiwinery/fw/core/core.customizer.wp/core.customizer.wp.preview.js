...
( function( $ ) {
	...
	if (!jardiwinery_previewer_vars['need_refresh']) {
		var $style = $('#jardiwinery-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<style type="text/css" id="jardiwinery-customizer-css"></style>' ).find('#jardiwinery-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			jardiwinery_set_cookie(jardiwinery_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
