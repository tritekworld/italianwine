...
<body <?php body_class();?>>
	...
	<?php get_template_part(jardiwinery_get_file_slug('sidebar_outer.php')); ?>

	<?php
		$body_style  = jardiwinery_get_custom_option('body_style');
		$class = $style = '';
		if (jardiwinery_get_custom_option('bg_custom')=='yes' && ($body_style=='boxed' || jardiwinery_get_custom_option('bg_image_load')=='always')) {
			if (($img = jardiwinery_get_custom_option('bg_image_custom')) != '')
				$style = 'background: url('.esc_url($img).') ' . str_replace('_', ' ', jardiwinery_get_custom_option('bg_image_custom_position')) . ' no-repeat fixed;';
			else if (($img = jardiwinery_get_custom_option('bg_pattern_custom')) != '')
				$style = 'background: url('.esc_url($img).') 0 0 repeat fixed;';
			else if (($img = jardiwinery_get_custom_option('bg_image')) > 0)
				$class = 'bg_image_'.($img);
			else if (($img = jardiwinery_get_custom_option('bg_pattern')) > 0)
				$class = 'bg_pattern_'.($img);
			if (($img = jardiwinery_get_custom_option('bg_color')) != '')
	...
	<div class="body_wrap<?php echo !empty($class) ? ' '.esc_attr($class) : ''; ?>"<?php echo !empty($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
		...
		<?php
		$video_bg_show = jardiwinery_get_custom_option('show_video_bg')=='yes';
		$youtube = jardiwinery_get_custom_option('video_bg_youtube_code');
		$video   = jardiwinery_get_custom_option('video_bg_url');
		$overlay = jardiwinery_get_custom_option('video_bg_overlay')=='yes';
		...
		<div class="page_wrap">
			...
			<?php
			$top_panel_style = jardiwinery_get_custom_option('top_panel_style');
			$top_panel_position = jardiwinery_get_custom_option('top_panel_position');
			$top_panel_scheme = jardiwinery_get_custom_option('top_panel_scheme');
			// Top panel 'Above' or 'Over'
			if (in_array($top_panel_position, array('above', 'over'))) {
				jardiwinery_show_post_layout(array(
					'layout' => $top_panel_style,
					'position' => $top_panel_position,
					'scheme' => $top_panel_scheme
					), false);
				// Mobile Menu
				get_template_part(jardiwinery_get_file_slug('templates/headers/_parts/header-mobile.php'));
			}

			// Slider
			get_template_part(jardiwinery_get_file_slug('templates/headers/_parts/slider.php'));
			...
			$show_title = jardiwinery_get_custom_option('show_page_title')=='yes';
			$show_navi = $show_title && is_single() && jardiwinery_is_woocommerce_page();
			$show_breadcrumbs = jardiwinery_get_custom_option('show_breadcrumbs')=='yes';
						...
						<div class="content_wrap">
							...
							if ($show_title) {
								if ($show_navi) {
									...
									?><h1 class="page_title"><?php echo strip_tags(jardiwinery_get_blog_title()); ?></h1><?php
								}
							}
							if ($show_breadcrumbs) {
								?><div class="breadcrumbs"><?php if (!is_404()) jardiwinery_show_breadcrumbs(); ?></div><?php
							}
							?>
						</div>
					</div>
			...
			<div class="page_content_wrap page_paddings_<?php echo esc_attr(jardiwinery_get_custom_option('body_paddings')); ?>">

				<?php
				// Content and sidebar wrapper
				if ($body_style!='fullscreen') jardiwinery_open_wrapper('<div class="content_wrap">');
				
				// Main content wrapper
				jardiwinery_open_wrapper('<div class="content">');
				?>