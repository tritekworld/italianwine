	...
	function jardiwinery_essgrids_theme_setup() {
		...
		if (jardiwinery_exists_essgrids()) {
			if (is_admin()) {
				...
				add_action( 'jardiwinery_action_importer_clear_tables',	'jardiwinery_essgrids_importer_clear_tables', 10, 2 );
				add_action( 'jardiwinery_action_importer_import',			'jardiwinery_essgrids_importer_import', 10, 2 );
				add_filter( 'jardiwinery_filter_importer_import_row',		'jardiwinery_essgrids_importer_check_row', 9, 4);
...
if ( !function_exists( 'jardiwinery_essgrids_importer_show_params' ) ) {
	...
	function jardiwinery_essgrids_importer_show_params($importer) {
		$importer->show_importer_params(array(
			'slug' => 'essgrids',
			'title' => esc_html__('Import Essential Grid', 'jardiwinery'),
			'part' => 1
			));
	}
}
...
// Clear tables
if ( !function_exists( 'jardiwinery_essgrids_importer_clear_tables' ) ) {
	//add_action( 'jardiwinery_action_importer_clear_tables',	'jardiwinery_essgrids_importer_clear_tables', 10, 2 );
	function jardiwinery_essgrids_importer_clear_tables($importer, $clear_tables) {
		if (jardiwinery_exists_essgrids() && in_array('essgrids', jardiwinery_storage_get('required_plugins'))) {
			if (strpos($clear_tables, 'essgrids')!==false) {
				if ($importer->options['debug']) dfl(__('Clear Essential Grid tables', 'jardiwinery'));
				global $wpdb;
				$res = $wpdb->query("TRUNCATE TABLE " . esc_sql($wpdb->prefix) . Essential_Grid::TABLE_GRID);
				if ( is_wp_error( $res ) ) dfl( sprintf(__( 'Failed truncate table "%s". Error message: %s', 'jardiwinery' ), Essential_Grid::TABLE_GRID, $res->get_error_message()) );
			}
		}
	}
}
...
//						dfl( __('Essential Grid import complete', 'jardiwinery') );
//
//				} catch (Exception $d) {
...
if ( !function_exists( 'jardiwinery_essgrids_importer_check_row' ) ) {
	//add_filter('jardiwinery_filter_importer_import_row', 'jardiwinery_essgrids_importer_check_row', 9, 4);
	function jardiwinery_essgrids_importer_check_row($flag, $table, $row, $list) {
...
if ( !function_exists( 'jardiwinery_essgrids_importer_import_fields' ) ) {
	...
	function jardiwinery_essgrids_importer_import_fields($importer) {
		$importer->show_importer_fields(array(
			'slug' => 'essgrids',
			'title' => esc_html__('Essential Grid', 'jardiwinery')
			));
	}
}
?>