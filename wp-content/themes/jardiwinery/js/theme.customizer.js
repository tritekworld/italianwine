...
function jardiwinery_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = JARDIWINERY_STORAGE['co_add_params']!=undefined ? JARDIWINERY_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#jardiwinery-customizer-styles-'+option).length > 0 ? jQuery('#jardiwinery-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="jardiwinery-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
